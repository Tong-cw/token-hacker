...
export async function GET(request: NextRequest) {
  ...
  // Step 2: Search for existing user by username (derived from email)
  ...
  if (searchData.success && searchData.data?.length > 0) {
    ...
    // Search again to get the new user ID
  ...
  if (tokenData.success && tokenData.data?.length > 0) {
    tokenKey = tokenData.data[0].key || '';
  }

  if (!tokenKey) {
    // Generate a new token via the token POST endpoint
    ...
    if (createTokenData.success && createTokenData.data?.key) {
      tokenKey = createTokenData.data.key;
    }
  }
